...
def generate_initial_credal_matrix(n, c):
    f = 2 ** c
    m = np.random.rand(n, f)
...
def wecm(x, c, g0=None, W=None, type='full', pairs=None, Omega=True, ntrials=1, alpha=1, beta=2, delta=10,
    ...
    # ------------------------ iterations--------------------------------
    # Initialize M and W -> compute new_V -> compute new_M and new_W for the next interation.
    Jbest = np.inf
    for itrial in range(ntrials):
        ...
        # Initialize credal matrix
        m = generate_initial_credal_matrix(n, c)
        ...
        while pasfini:
            ...
            g = V
            # Calculate (2^c - 1) centroids gplus for the next iteration
            for i in range(1, f):
                fi = F[i, :]
                truc = np.tile(fi, (d, 1)).T
                gplus[i - 1, :] = np.sum(g * truc, axis=0) / np.sum(fi)

            # calculation of distances to the next centers (n x (2^c - 1))
            ...
            # Calculation of masses for the next iteration
            next_m = np.zeros((n, f - 1))
            for i in range(n):
                vect0 = DW[i, :]
                for j in range(f - 1):
                    ...
                    vect3 = vect1 * vect2
                    next_m[i, j] = 1 / (np.sum(vect3) + (card[j] ** alpha * DW[i, j] / delta2) ** (1 / (beta - 1)))
                    if np.isnan(next_m[i, j]):
                        next_m[i, j] = 1  # in case the initial prototypes are training vectors

            # Calculate new weights for the next iteration using current W, M and V
            W = projected_gradient_descent_method(start_W, M=next_m, V=gplus, F=F, X=x, alpha=alpha, beta=beta, delta=delta,
                                                  iterations=100,
                                                  stopping_threshold=epsi)

            # Calculate objective function's new value
            # J = get_j1_objective_func_value(start_W, m, gplus, F, x, alpha, beta, delta)
            J = get_j1_objective_func_value(W, next_m, gplus, F, x, alpha, beta, delta)
