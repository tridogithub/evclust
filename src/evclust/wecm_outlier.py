...
def finding_new_weights(old_W, M, V, F, X, alpha, beta, delta, noise_w):
    """
    ...
    iterations = 50
    new_W = None
    old_J = get_j1_objective_func_value(old_W, M, V, F, X, alpha, beta, delta, noise_w)
    # finding the step length t in {1, gamma^1, gamma^2, ...}
    while not finis:
        new_W = old_W + t * D

        new_J = get_j1_objective_func_value(new_W, M, V, F, X, alpha, beta, delta, noise_w)
...
def projected_gradient_descent_method(start_W, M, V, F, X, alpha, beta, delta, noise_w, iterations=100,
    ...
    for i in range(iterations):
        W = finding_new_weights(start_W, M, V, F, X, alpha, beta, delta, noise_w)
