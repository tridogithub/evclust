...
"""
This module contains the main function for w-ecm (Weighted ECM clustering) with new equation of barycenters, and the exponent of weight in distances to calculate membership is set to 1
"""
...
def __get_objective_func_value(w, m, v, F, x, alpha, beta, delta, noise_w):
    n = m.shape[0]
    f = F.shape[0]
    ...
    j1 = np.nansum((m ** beta) * dw2[:, :f - 1] * (np.tile(card[:f - 1] ** alpha, (n, 1))) + (
                np.sum(noise_w ** 2) * delta ** 2) * np.nansum(mvide[:f - 1] ** beta))
    return j1


def __get_noise_cluster_weights(x):
    """
    Args:
...
def wecm(x, c, v0=None, alpha=1, beta=2, delta=10, epsilon=1e-3, stopping_factor=None, init="kmeans", disp=True):
    ...
    noise_w = __get_noise_cluster_weights(x)
    ...
    while not finis:
        ...
        # Calculate weighted distances to calculate memberships
        dw2 = np.zeros((n, f - 1))
        for j in range(f - 1):
            wj = np.tile(w0[j, :], (n, 1))  # Weight of each dimension wrt cluster j, repeated n times
            dw2[:, j] = np.nansum((x - np.tile(vplus[j, :], (n, 1))) ** 2 * wj, axis=1)
        ...
        for i in range(n):
            ...
            for j in range(f - 1):
                ...
                vect3 = vect1 * vect2
                m[i, j] = 1 / (np.sum(vect3) + (card[j] ** alpha * dw2[i, j] / (delta2 * np.sum(noise_w ** 2))) ** (
                        1 / (beta - 1)))
                if np.isnan(m[i, j]):
        ...
        J = __get_objective_func_value(w, m, v, F, x, alpha, beta, delta, noise_w)
        iteration += 1
        if disp:
            print([iteration, J])
